let player1 = [[3,10],[10,3]];
let player2 = [[2,1],[10,8],[3,6],[3,10],[7,6],[1,7],[3,5],[5,6],[8,1],[7,6]]; 
function battle(p1, p2) {
    let res = {
        player1: [],
        player2: [], 
    };
    res.player1 = p1.reduce((arr, item, i) => {
        if (p2[i] === undefined) {
            arr.push(item);
        }
        else 
        if (item[0] + item[1] > p2[i][0] + p2[i][1]) {
            arr.push(item);
        }
        return arr; 
    },[]);
    res.player2 = p2.reduce((arr, item, i) => {
        if (p1[i] === undefined) {
            arr.push(item);
        } else 
        if  (item[0] + item[1] > p1[i][0] + p1[i][1]) {
            arr.push(item);
        }
        return arr;
    },[]);
