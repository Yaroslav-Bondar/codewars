inArr = [3,"3","2",2,"2","1",1,"a","b","c"];
function unusualSort(arr){
    let letterArr;    
    letterArr = arr.filter(item => isNaN(item)).sort().concat(arr.filter(item => !isNaN(item)).sort((a, b) => { if(a > b) return 1; if(a < b) return -1; if(a == b && typeof b === 'string') return -1;}));
...
You may assume that argument will always be an array/list of characters or one-digit-long */
