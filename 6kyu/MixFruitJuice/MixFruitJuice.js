function mixFruit (arr) {
    var price = {
        ...
        strawberry: 7,
        mango: 7,
        // getSumPrice: () => console.log(Math.round(arr.reduce( (countPrice,item) => {
        //     item=item.toLowerCase();
        //     return item in price ? countPrice+price[item] : countPrice+9;  // method of reduce
        // },0)/arr.length)) 
        getSumPrice: () => { return (Math.round(arr.reduce( (countPrice,item) => {
            item=item.toLowerCase(); 
            return item in price ? countPrice+price[item] : countPrice+9; },0) / arr.length)) }
    };
    // var countPrice=0, amountFruits=0;
    // for (var i=0; i<arr.length; i++) {
    //     if (arr[i] in price) {
    //         // console.log(arr[i]);
    //         countPrice+=price[arr[i]];
    //         amountFruits++;                                 classic method
    //     }
    //     else {
    //         countPrice+=9;
    //         amountFruits++;
    ...
    // }
    // console.log(price.g);
    // console.log(Math.round(countPrice/amountFruits));

    // console.log(Math.round(arr.reduce( (countPrice,item) => {
    //     item=item.toLowerCase();
    //     return item in price ? countPrice+price[item] : countPrice+9;  // method of reduce
    // },0)/arr.length));

    price.getSumPrice(); // object property call

    // return Math.round(arr.reduce( (countPrice,item) => item in price ? countPrice+price[item] : countPrice+9,0) /arr.length);  
    //short method of reduce 
}
// mixFruit(['banana','mango','avocado']);
mixFruit(['watermelon','Banana','avocado']);
