function decode  (code, n) {
    let letter = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"], dropKey, mess;
    var i = 0;
    dropKey = n.toString().split('').map(item => Number(item));
    mess = code.reduce((mes, val) => {
        if(i == dropKey.length)
            i = 0;
        mes += letter[val - dropKey[i] - 1];    
        i++;
        return mes;
    },'');
    console.log(mess);
